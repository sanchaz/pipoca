...
%token <exp>EXP
%token <i>INT
%token INIT2 INIT3 EXTERN2 MULTIIDS CONST2 FUNC UNALLOCVECTOR FUNC2 IDVEC IDVECINT IDVECID EXPP FUNCCALL IDVECA PLUSONEA MINUSONEA MEMRESERV MEMRESERVP SSM LISTEXP LISTEXP2 LISTEXP3 LISTEXP4 DECL INITN DEC INTS EXP4 MEMRESERV PRINT PRINTP PRINTM PRINTSM PRINTINT PRINTID PRINTSTRING PRINTFUNCCALL FID INITID INTVEC ICONST SCONST VCONST NEG EXTERNFUNC EXTERNFUNCARGS
%type <n> pipoca file decl init init2 init3 extern extern2 extern3 const2 const func listexp3 listexp4 listexp listexp2 func2 breakcontinue dec type var ints elseif endelseif elsecycle endelsecycle exp4 exp5 exp6 exp exp2 exp3 memreserv memreserv2 memreserv3 memreserv4 print print2 print3 intorreal
%left AND
%left OR
%left '>' '<' NOTEQUAL MOREOREQUAL LESSOREQUAL
%left '='
%left '-' '+'
%left '*' '/' '%'
...
decl:		init	{$$ = $1;}
	...
	|	NEWLINE	{$$ = 0;}
	;
init:		INIT ID OPATR init2 NEWLINE	{$$ = subNode(INIT, 2, strNode(INITID, $2), $4);}
	;
...
init3:		intorreal	{$$ = subNode(INITN, 1, $1);}
	|	'-' intorreal	{$$ = subNode(INITN, 1, subNode(NEG, 1, $2));}
	|	init3 ',' intorreal	{$$ = addNode($1, $3, $1->value.sub.num);}
	|	init3 ',' '-' intorreal	{$$ = addNode($1, subNode(NEG, 1, $4), $1->value.sub.num);}
	;
		
extern:		EXTERN type extern2 NEWLINE	{$$ = $3;}
	;
			
extern2:	ID	{$$ = subNode(EXTERN, 1, strNode(ID, $1));}
	|	ID '(' extern3 ')'	{$$ = subNode(EXTERN, 1, subNode(EXTERNFUNC, 2, strNode(FID, $1), $3));}
	|	ID '(' ')'	{$$ = subNode(EXTERN, 1, subNode(EXTERNFUNC, 1, strNode(FID, $1)));}
	|	extern2 ',' ID	{$$ = addNode($1, strNode(ID, $3), $1->value.sub.num);}
	|	extern2 ',' ID '(' extern3 ')'	{$$ = addNode($1, subNode(EXTERNFUNC, 2, strNode(FID, $3), $5), $1->value.sub.num);}
	|	extern2 ',' ID '(' ')'	{$$ = addNode($1, subNode(EXTERNFUNC, 1, strNode(FID, $3)), $1->value.sub.num);}
	;
	
extern3:	ID	{$$ = subNode(EXTERNFUNCARGS, 1, strNode(ID, $1));}
	|	extern ',' ID	{$$ = addNode($1, strNode(ID, $3), $1->value.sub.num);}
	;

const2:		ID OPATR const	{$$ = subNode(CONST, 1, subNode(OPATR, 2, strNode(ID, $1), $3));}
...
listexp3:		{$$ = 0;}
		|	ID	{$$ = subNode(LISTEXP3, 1, strNode(ID, $1));}
		|	ID '[' ']'	{$$ = subNode(LISTEXP3, 1, subNode(UNALLOCVECTOR, 1, strNode(ID, $1)));}
		|	listexp4 ',' ID '[' ']'	{$$ = subNode(LISTEXP3, 2, $1, subNode(UNALLOCVECTOR, 1, strNode(ID, $3)));}
		|	listexp4 ',' ID	{$$ = subNode(LISTEXP3, 2, $1, strNode(ID, $3));}
		;
			
listexp4:		ID	{$$ = subNode(LISTEXP4, 1, strNode(ID, $1));}
		|	ID '[' ']'	{$$ = subNode(LISTEXP4, 1, subNode(UNALLOCVECTOR, 1, strNode(ID, $1)));}
		|	listexp4 ',' ID '[' ']'	{$$ = subNode(LISTEXP4, 2, $1, subNode(UNALLOCVECTOR, 1, strNode(ID, $3)));}
		|	listexp4 ',' ID	{$$ = subNode(LISTEXP4, 2, $1, strNode(ID, $3));}
		;

listexp:	{$$ = 0;}
	|	exp5	{$$ = subNode(LISTEXP2, 1, $1);}
	|	listexp2 ',' exp5	{$$ = subNode(LISTEXP, 2, $1, $3);}
	;
			
listexp2:		exp5	{$$ = subNode(LISTEXP2, 1, $1);}
		|	listexp2 ',' exp5	{$$ = subNode(LISTEXP2, 2, $1, $3);}
		;
...
elsecycle:		ELSE NEWLINE STARTBLOCK func2 ENDBLOCK endelsecycle	{$$ = subNode(ELSE, 1, $4);}
		|	endelsecycle	{$$ = $1;}
		;
		
elseif:		ELSE NEWLINE STARTBLOCK func2 ENDBLOCK endelseif	{$$ = subNode(ELSE, 1, $4);}
	|	endelseif	{$$ = $1;}
	;
...
exp6:		exp5			{$$ = $1;}
	...
	|	ID '[' memreserv2 ']' OPATR exp6	{$$ = subNode(OPATR, 2, subNode(IDVECA, 2, strNode(ID, $1), $3), $6);}
	;

exp:		exp2	{$$ = $1;}
	|	exp3	{$$ = $1;}
	|	exp '-' exp	{$$ = subNode('-', 2, $1, $3);}
	|	exp '+' exp	{$$ = subNode('+', 2, $1, $3);}
	|	exp '*' exp	{$$ = subNode('*', 2, $1, $3);}
	|	exp '/' exp	{$$ = subNode('/', 2, $1, $3);}
	|	exp '%' exp	{$$ = subNode('%', 2, $1, $3);}
	|	exp POT exp	{$$ = subNode(POT, 2, $1, $3);}
	|	exp '<' exp	{$$ = subNode('<', 2, $1, $3);}
	|	exp '>' exp	{$$ = subNode('>', 2, $1, $3);}
	|	exp '=' exp	{$$ = subNode('=', 2, $1, $3);}
	|	exp MOREOREQUAL exp	{$$ = subNode(MOREOREQUAL, 2, $1, $3);}
	|	exp LESSOREQUAL exp	{$$ = subNode(LESSOREQUAL, 2, $1, $3);}
	|	exp NOTEQUAL exp	{$$ = subNode(NOTEQUAL, 2, $1, $3);}
	;
...
exp3:		ID				{$$ = strNode(ID, $1);}
	...
	|	'@' ID '[' memreserv2 ']'	{$$ = subNode('@', 1, subNode(IDVECA, 2, strNode(ID, $2), $4));}
	|	ID '(' listexp ')'		{$$ = subNode(FUNCCALL, 2, strNode(FID, $1), $3);}
	...
	|	PLUSONE ID '[' memreserv2 ']'	{$$ = subNode(PLUSONE, 1, subNode(IDVECA, 2, strNode(ID, $2), $4));}
	|	MINUSONE ID '[' memreserv2 ']'	{$$ = subNode(MINUSONE, 1, subNode(IDVECA, 2, strNode(ID, $2), $4));}
	|	ID PLUSONE			{$$ = subNode(PLUSONEA, 1, strNode(ID, $1));}
	|	ID MINUSONE			{$$ = subNode(MINUSONEA, 1, strNode(ID, $1));}
	|	ID '[' memreserv2 ']' PLUSONE	{$$ = subNode(PLUSONEA, 1, subNode(IDVECA, 2, strNode(ID, $1), $3));}
	|	ID '[' memreserv2 ']' MINUSONE	{$$ = subNode(MINUSONEA, 1, subNode(IDVECA, 2, strNode(ID, $1), $3));}
	|	intorreal			{$$ = $1;}
	|	NOT ID				{$$ = subNode(NOT, 1, strNode(ID, $2));}
	|	NOT ID '[' memreserv2 ']'	{$$ = subNode(NOT, 1, subNode(IDVECA, 2, strNode(ID, $2), $4));}
	|	'-' ID				{$$ = subNode(NEG, 1, strNode(ID, $2));}
	|	'-' ID '[' memreserv2 ']'	{$$ = subNode(NEG, 1, subNode(IDVECA, 2, strNode(ID, $2), $4));}
	|	'-' intorreal			{$$ = subNode(NEG, 1, $2);}
	|	ID '?'				{$$ = subNode('?', 1, strNode(ID, $1));}
	|	ID '[' memreserv2 ']' '?'	{$$ = subNode('?', 1, subNode(IDVECA, 2, strNode(ID, $1), $3));}
	;
...
print2:		STRING	{$$ = strNode(PRINTSTRING, $1);}
	|	print2 '+' ID	{$$ = subNode(PRINTP, 2, $1, subNode(PRINTID, 1, strNode(ID, $3)));}
	|	print2 '+' ID '[' intorreal ']'	{$$ = subNode(PRINTP, 2, $1, subNode(PRINTID, 1, subNode(IDVECA, 2, strNode(ID, $3), $5) ));}
	|	print2 '+' ID '(' listexp ')'	{$$ = subNode(PRINTP, 2, $1, subNode(PRINTFUNCCALL, 1, subNode(FUNCCALL, 2, strNode(FID, $3), $5)));}
...
print3:		ID	{$$ = strNode(ID, $1);}
	|	intorreal	{$$ = $1;}
	|	print3 '*' ID	{$$ = subNode('*', 2, $1, strNode(ID, $3));}
	|	print3 '*' intorreal	{$$ = subNode('*', 2, $1, $3);}
	;
